...
import model.fan.braqued.NameGet;
...
public class Koans {
    ...
    public void koans() {
        Assert.assertTrue("Truth will set you free. Or, in less philosophical terms, " +
                ...
                "In doing so, you will learn how braque works.", true/*false*/);
        ...
        test4Map.put("baseballplayerinfo/myId/_type", "BaseballPlayer"/*"bogusType"*/);
        ...
        test5Map.put("baseballplayerinfo/myId/_type", "BaseballPlayer"/*"bogusType"*/);
        ...
        MeUpdateFan aFan = Transformer.makeMeUpdateFan("aFanId").addName("Bob")/**/.addHometown("New York").commit();
        ...
        Assert.assertTrue("If you want to access properties of objects created by the transformer,"+
                ...
                "Let's fix that by casting to the right Get interface and calling the right get method. "+
                "Don't cheat by changing the name to the hometown.", (/**/(NameGet)anotherFan).getName().equals("Joe Smith"));
        ...
        Assert.assertTrue("We can avoid casts by using the type returned by commit.  This is easily found by most modern IDEs. "+
                "The naming convention is all of the asked-for properties in alphabetical order separated by an underscore.", (/**/yetAnotherFan.getName().equals("Joe Smith")));

        MeUpdateFan aFanWithoutANameOrAHometown = Transformer.take(yetAnotherFan).removeName().removeHometown().commit();
        ...
        Assert.assertTrue("@Clojure allows us to inject code into our objects using Clojure code. "+
                ...
                areThereAnyDeprecatedFields(/*BaseballPlayerInfoShowPlayerNumbersWorn_Implementation*/PlayersShowPlayerNumbersWorn_Position_Implementation.class));
